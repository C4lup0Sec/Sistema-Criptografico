...
def login(privada: bytes, certificado: bytes, password: str):
    global valor
    #Cifra el resultado del cifrado césar
    global contra_ces
    ...
    try:
        ...
        print(colored("Bienvenido a la ventana de mensajería", "green", attrs=["bold"]))
...
#Cuando valida al usuario muestra el siguiente menú
def opc_log(val):
    while(val == True):
        ...
        if eleccion == 1:   
            print(colored("Este mensaje se cifrara por Cesar", "red", attrs=["bold"]))
            mensaje = input(colored("Escribe tu mensaje > ", "blue", attrs=["bold"]))
            #Se envian el mensaje y su llave para encriptar
            enc_msj(mensaje, 3)
        elif eleccion == 2:
            print(colored("Copia y pega abajo el mensaje encriptado", "red", attrs=["bold"]))
            decode = input(colored("Ingresa el mensaje encriptado > ", "blue", attrs=["bold"]))
            desenc_msj(decode, 3)
        elif eleccion < 1 or eleccion > 2:
            val = False

#Esto es para encriptar el mensaje   
...
#Esto es para desencriptar el mensaje   
        ...
        if opc == 1:            
            while(True):
                ...
                if not certificado or not privada or not password:
                    ...
                    #Aqui se comprueba la existencia del usuario
                    login(privada, certificado, password)
                    break
            #Se envia el valor que se obtiene al validar el usuario
            opc_log(valor)
        #Si presiona 2 dirige al registro
        if opc == 2:            
            #Esto se usa para verificar los datos ingresados
            while(True):
